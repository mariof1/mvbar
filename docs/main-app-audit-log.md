...
### Follow-up: asynchronous backup failures without WebSocket

Confirmed Admin issue: if the real-time connection was unavailable, the server's asynchronous backup job could fail after POST accepted it, and the polling list would only clear the spinner. The administrator saw no failure reason. An isolated browser fixture reproduced the missing error against the previous port 8080 build. The admin-only list endpoint now includes the most recent creation job result (success or failure and an error for failures), keyed by job ID. The web panel matches that result to the job it started or observed and shows a persistent failure without relying on the WebSocket toast. A new job clears the previous result; normal list refreshes do not erase operation errors.

A rejected restore fixture also confirmed the previous code stayed in Admin, showed the API's specific archive error, and re-enabled Restore; no change to that flow was necessary. Six intercepted backup fixtures passed in the isolated updated production preview, including prior list race/retry and download cases. After deployment, seven fixtures passed against port 8080, covering both failed and successful asynchronous creation without WebSocket events. Web TypeScript, web/API targeted ESLint, API build, eight backup unit checks, and web production builds passed. The local server was restarted after the Admin page showed no active scan or player and returned to ready. No real backup was created, restored, uploaded, or deleted. This in-memory job result does not survive an API process restart; feedback after a restart still relies on persisted archives and the normal WebSocket when available. Continue with other main-app flows and the authenticated two-web-player Connect gap.

### Follow-up: podcast subscription and directory errors

Confirmed the web subscribe modal discarded a specific API feed-validation error for both a direct RSS URL and a podcast-directory search result. The direct RSS fixture showed the generic "API error" instead of the returned reason. A separate search fixture showed "No podcasts found" alongside a directory-unavailable error, implying a valid empty result after a failed request. The modal now reads the API error body for both subscription methods and only shows the empty-results message after a successful search.

Three isolated browser fixtures for invalid RSS feeds, search-result subscription failure, and directory failure passed against an updated production preview and the deployed port 8080 build. The existing podcast seek/completion fixture also passed in both environments. Web TypeScript, targeted ESLint, and production web builds passed. API traffic and WebSocket were intercepted; no real subscriptions, episodes, downloads, or playback changed. The Admin page showed no active scan or player before deployment and the local server returned to ready. Continue with other podcast download/error flows and the authenticated two-web-player Connect gap.
