...
async function periodicRescan(force = false) {
    if (scanInProgress) {
...
subscriber.subscribe('library:commands', (err) => {
    if (err)
        logger.error('worker', `Failed to subscribe to commands: ${err.message}`);
    else
        logger.info('worker', 'Listening for rescan commands');
});
