...
function safeJoinMount(mountPath, relPath) {
...
export async function transcodeTrackToHls(trackId, cacheKey) {
    const r = await db().query('select t.path, l.mount_path from tracks t join libraries l on l.id=t.library_id where t.id=$1', [trackId]);
    const row = r.rows[0];
    if (!row)
