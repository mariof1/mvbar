  ...
  return withRetry(async () => {
    ...
    try {
      ...
      if (genre) {
        ...
        if (genres.length > 0) {
          await client.query(
            `insert into track_genres(track_id, genre) select $1, unnest($2::text[]) on conflict do nothing`,
            [trackId, genres]
          );
        }
      }
      ...
      if (country) {
        ...
        if (countries.length > 0) {
          await client.query(
            `insert into track_countries(track_id, country) select $1, unnest($2::text[]) on conflict do nothing`,
            [trackId, countries]
          );
        }
      }
      ...
      if (language) {
        ...
        if (languages.length > 0) {
          await client.query(
            `insert into track_languages(track_id, language) select $1, unnest($2::text[]) on conflict do nothing`,
            [trackId, languages]
          );
        }
      }
