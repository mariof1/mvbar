...
test('Connect isolation, controls, transfer confirmation, queue mapping and disconnects', async (t) => {
  ...
  await destination.wait('connect:command');
  const replacement = await client('destination');
  ...
  controller.send('connect:transfer', { commandId: 'in-flight', sourceDeviceId: 'source', targetDeviceId: 'destination' });
  await replacement.wait('connect:command', d => d.commandId === 'in-flight');
  controller.send('connect:transfer', { commandId: 'overlap', sourceDeviceId: 'source', targetDeviceId: 'controller' });
  assert.equal((await controller.wait('connect:command_ack', d => d.commandId === 'overlap')).accepted, false);
  source.send('connect:state', { queue: [{ id: 2 }], queueIndex: 0, isPlaying: true });
  await controller.wait('connect:devices', d => d.devices.some(x => x.id === 'source' && x.state.track?.id === 2));
  replacement.send('connect:command_result', { commandId: 'in-flight', success: true });
  const changed = await controller.wait('connect:command_ack', d => d.commandId === 'in-flight');
  assert.equal(changed.accepted, false);
  await replacement.wait('connect:command', d => d.commandId === 'in-flight:cancel' && d.command === 'pause');
  assert.equal(source.messages.some(m => m.type === 'connect:command' && m.data.commandId === 'in-flight:pause'), false);
