...
function queryMentionsTerm(query: string, term: string): boolean {
  const comparable = (value: string) => normalizeFeature(value)
    .replace(/[^a-z0-9]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  const haystack = comparable(query);
  const needle = comparable(term);
  if (!haystack || !needle) return false;
  return ` ${haystack} `.includes(` ${needle} `);
}

function queryExplicitlyNamesGenre(query: string, terms: string[]): boolean {
  const queryWords = new Set(normalizeFeature(query).split(/[^a-z0-9]+/).filter((word) => word.length >= 4));
  return terms.some((term) => {
    if (queryMentionsTerm(query, term)) return true;
    const termWords = normalizeFeature(term).split(/[^a-z0-9]+/).filter((word) => word.length >= 4);
    return termWords.some((word) => queryWords.has(word));
  });
}

function explicitExclusions(query: string): { terms: string[]; phrases: string[] } {
  const terms: string[] = [];
  const phrases: string[] = [];
  const pattern = /\b(?:exclude|excluding|without|except(?:\s+for)?|but\s+(?:not|no)|avoid|skip|leave\s+out|(?:do\s+not|don['’]?t)\s+(?:play|include|add|use|want(?:\s+to\s+hear)?)|not\s+(?:songs?|tracks?|music)?\s*by|no\s+(?:songs?|tracks?|music)\s+by)\s+(.+?)(?=\s+(?:but|while|instead|then|and\s+(?:play|queue|add|include|find|search))\b|[.;]|$)/gi;

  const addClause = (fullPhrase: string, rawClause: string) => {
    phrases.push(fullPhrase);
    const clause = rawClause
      .replace(/^[\s'"“”]*(?:(?:any|the\s+artist|artist|the\s+band|band|songs?\s+by|tracks?\s+by|music\s+by)\s+)?/i, '')
      .replace(/\s+(?:from|in)\s+(?:this|the)\s+(?:mix|playlist|queue|results?)\s*$/i, '')
      .replace(/\s+(?:songs?|tracks?|music)\s*$/i, '')
      .replace(/\s+(?:again|please)\s*$/i, '')
      .replace(/[\s'"“”]+$/g, '');
    for (const item of clause.split(/\s*(?:,|;|\bor\b)\s*/i)) {
      const term = normalizedTerm(item);
      const genericReference = /^(?:by\s+)?(?:(?:the\s+)?(?:original|seed|same)(?:\s+(?:artist|band))?|(?:the\s+|that\s+)?(?:artist|band)|him|her|them|it)$/i.test(term);
      if (term && !genericReference) terms.push(term);
    }
  };

  for (const match of query.matchAll(pattern)) {
    addClause(match[0], match[1]);
  }

  // Also understand the post-positive form: “leave Taco Hemingway out”.
  for (const match of query.matchAll(/\bleave\s+(.+?)\s+out\b/gi)) {
    addClause(match[0], match[1]);
  }

  return { terms: Array.from(new Set(terms)).slice(0, 12), phrases };
}
...
export function fallbackMusicIntent(query: string): MusicIntent {
  ...
  const exclusions = explicitExclusions(query);
  let moods: string[] = [];
  let genres: string[] = [];
  ...
  avoid = mergeTerms(avoid, exclusions.terms);
  semanticWords.push(...quantityAndDuration.semanticWords, ...exclusions.phrases, 'each');
...
export function parseMusicIntent(content: string, fallbackQuery: string): MusicIntent {
  ...
  try {
    ...
    // Playback is a side effect, so it must be derived from the user's own
    // wording. Never let a model turn a descriptive search into auto-play.
    const action = inferAction(fallbackQuery);
    ...
    const parsedTextQuery = cleanText(typeof parsed.textQuery === 'string' ? parsed.textQuery : '', 160);
    ...
    const modelCountries = normalizeCountryTerms(parsed.countries);
    const countries = fallback.countries.length > 0
      ? fallback.countries
      : modelCountries.filter((country) => queryMentionsTerm(fallbackQuery, country));
    const namedArtists = normalizedTerms(parsed.namedArtists, 10)
      .filter((artist) => queryMentionsTerm(fallbackQuery, artist));
    ...
    const hasSemanticInterpretation = moods.length > 0 || genres.length > 0 || relatedGenres.length > 0
      || namedArtists.length > 0 || similarToArtists.length > 0 || referenceArtists.length > 0;
    const identityQuery = textQueryWithoutFiller(fallbackQuery, []);
    const textQueryMatchesFallback = Boolean(parsedTextQuery && identityQuery
      && queryMentionsTerm(identityQuery, parsedTextQuery)
      && queryMentionsTerm(parsedTextQuery, identityQuery));
    const hasExplicitIdentityCue = /\b(?:song|track|album|artist|band|called|titled|by)\b/i.test(fallbackQuery);
    const textQuery = countryWestern
      ? ''
      : (parsedTextQuery && queryMentionsTerm(fallbackQuery, parsedTextQuery)
          && (textQueryMatchesFallback || hasExplicitIdentityCue)
        ? parsedTextQuery
        : (hasSemanticInterpretation ? '' : fallback.textQuery));
    ...
    // Explicit exclusions are parsed from the user's own words in fallback.
    // Always retain them even when the model supplies a different avoid list.
    const excludesSimilaritySeed = /\b(?:(?:do\s+not|don['’]?t)\s+(?:play|include|add|use|want(?:\s+to\s+hear)?)|exclude|excluding|without|except(?:\s+for)?|avoid|skip|leave\s+out|but\s+(?:not|no)|not\s+by)\s+(?:the\s+)?(?:original|seed|same)?\s*(?:artist|band|him|her|them|that\s+artist)\b/i.test(fallbackQuery);
    const resolvedAvoid = mergeTerms(
      fallback.avoid,
      normalizedTerms(parsed.avoid),
      excludesSimilaritySeed ? similarToArtists : []
    );
    ...
    return applyEnergyDefaults({
      ...
      requireGenreMatch: fallback.requireGenreMatch
        || (parsed.requireGenreMatch === true && queryExplicitlyNamesGenre(fallbackQuery, [...resolvedGenres, ...resolvedRelatedGenres])),
      countries: countries.length > 0 ? countries : fallback.countries,
      countryMode: fallback.countryMode,
      // These are hard constraints. They must come from deterministic parsing
      // of the user's words, never from a model's creative interpretation.
      yearStart: fallback.yearStart,
      yearEnd: fallback.yearEnd,
      ...
      similarArtists: [],
      includeSimilar: fallback.includeSimilar,
      avoid: resolvedAvoid,
      energy,
      ...
      minDurationMinutes: fallback.minDurationMinutes,
      maxDurationMinutes: fallback.maxDurationMinutes,
...
async function loadSemanticCandidates(
  ...
  return result.rows.map((row) => {
    return {
      ...
      displayArtist: artistDisplay(row.artist, row.album_artist),
      album: row.album,
      path: row.path,
      ext: row.ext,
...
function isRetryableProviderError(error: unknown): boolean {
  ...
  const status = 'status' in error ? Number((error as Error & { status?: number }).status) : 0;
  const openRouterStatus = 'openRouterStatus' in error
    ? Number((error as Error & { openRouterStatus?: number }).openRouterStatus)
    : 0;
  return status >= 500 || openRouterStatus === 429 || openRouterStatus === 503;
}
      ...
      messages: [
        {
          ...
          content: [
            ...
            'Never invent hard constraints: year, country, quantity and per-track duration must be null/default unless the user explicitly states them.',
            'Treat exclude, excluding, without, except and but not as mandatory. Put the exact excluded artist or style in avoid even when it is also the similarity seed.',
...
export async function requestMusicIntent(
  ...
  try {
    const result = await requestMusicIntentOnce(apiKey, model, query, signal, isFreeModel(model));
    ...
    if (model !== freeModel && (openRouterStatus === 402 || retryable)) {
      const result = await requestMusicIntentOnce(apiKey, freeModel, query, signal, true);
      return { ...result, usedFreeFallback: true };
    }

    // The free router can occasionally select a provider that returns an
    // empty or transiently unavailable response. One retry gives it a chance
    // to route to a different compatible provider without multiplying load.
    if (retryable) {
      const result = await requestMusicIntentOnce(apiKey, model, query, signal, isFreeModel(model));
      return { ...result, usedFreeFallback: false };
    }
...
export const aiPlugin: FastifyPluginAsync = fp(async (app) => {
  app.post('/api/ai/intent', async (req, reply) => {
    ...
    const apiKey = keyResult.rows[0]?.openrouter_api_key?.trim() || process.env.OPENROUTER_API_KEY?.trim();
    if (!apiKey) {
      return reply.code(400).send({
        ok: false,
        error: 'OpenRouter is not configured. Add an API key in Settings → Integrations or configure one on the server.',
      });
    }
