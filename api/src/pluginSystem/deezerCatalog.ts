...
export type DeezerAlbum = {
  ...
  publisher: string | null; barcode: string | null; gainDb: number | null;
};
...
type RawAlbum = {
  ...
  label?: string; upc?: string; gain?: number;
};
type RawTrack = {
...
export function deezerBaseAlbumTitle(value: string) {
  ...
  base = base.replace(/\s*[-–—:]?\s*(?:(?:19|20)\d{2}\s+)?remaster(?:ed)?(?:\s+(?:edition|version))?\s*$/i, '');
  base = base.replace(/\s*[-–—:]?\s*remaster(?:ed)?\s+(?:19|20)\d{2}\s*$/i, '');
  for (const suffix of EDITION_SUFFIXES) {
    base = base.replace(new RegExp('\\s*[-–—:]?\\s*' + esc(suffix) + '\\s*$', 'i'), '');
  }
...
function albumGain(raw: RawAlbum) {
